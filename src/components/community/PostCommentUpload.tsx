...
import { View, StyleSheet, Text, ActivityIndicator, TouchableOpacity, Button, TextInput, Alert, Image } from 'react-native';
import type { NativeStackScreenProps } from '@react-navigation/native-stack';

import styled from 'styled-components/native';
import * as ImagePicker from 'react-native-image-picker';

import { BoardFormat } from '../../pages/Community'
...
const PostCommentUploadSection = ({ comment_id, post_id, board_id, boardFormat, isParent, parentId, navigation, reRenderScreen, prevComment }: PostCommentUploadSectionProps) => {
    ...
    const uploadPostComment = async () => {
        ...
        if (comment_id) { // 수정
            await request.put(`/community/post_comments/${comment_id}/update`, formData, { "Content-Type": "multipart/form-data" })
        }
        else { // 생성
            await request.post('/community/post_comments/create/', formData, { "Content-Type": "multipart/form-data" });
        }
        setContent('');
        setPhotos([]);
        reRenderScreen();
        ...
        async function _getData() {
            try {
                ...
                if (comment_id && prevComment) {
                    ...
                    if (prevComment.photoList)
                        setPhotos(prevComment.photoList);
                }
                setLoading(false);
            }
            catch (err) {
                ...
                <>
                    <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                        <PostCommentInput
                            ...
                            value={content}
                        />
                        <TouchableOpacity style={{ marginRight: 10 }} onPress={async () => await uploadPostComment()}>

...
const FormHeader = ({ title, onLeft, onRight, begin, end }: FormHeaderProps) => {
  ...
  return (
    <View style={{height: Platform.OS === 'ios' ? height * 0.12 : height * 0.08, backgroundColor: '#67D393', flexDirection: 'row', justifyContent: 'center', alignItems: 'flex-end'}}>
      <TouchableOpacity style={{height: '100%', paddingBottom: 18, flex: 1, justifyContent: 'flex-end', alignItems: 'flex-start', paddingLeft: 10}} onPress={onLeft}>
        {begin ? <Close width={18} height={18} color={'white'} strokeWidth={3} style={{marginLeft: 5}} /> : <Arrow width={20} height={20} transform={[{ rotate: '180deg' }]} color="white" />}
      </TouchableOpacity>
      ...
      <TouchableOpacity style={{height: '100%', paddingBottom: 18, flex: 1, justifyContent: 'flex-end', alignItems: 'flex-end', paddingRight: 10}} onPress={onRight}>
        {end ? <Text style={{color: 'white', fontWeight: '500', lineHeight: 20, marginRight: 5}}>완료</Text> : <Arrow width={20} height={20} color="white" />}
      </TouchableOpacity>
    </View>
  )
}
